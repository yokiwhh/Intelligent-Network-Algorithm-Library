...
class DatanetAPI:
    ...
    类，包含通过迭代操作逐行读取数据集的所有功能，并使用收集到的信息生成一个Sample实例。
    """
    
    def __init__ (self, data_folder, intensity_values = [], topology_sizes = [],
                  shuffle=False):
        """
        Initialization of the Pasring Tool instance
        ...
        for root, dirs, files in os.walk(self.data_folder):
            # root是“...\train\”，指的是当前正在遍历的这个文件夹的本身的地址
            # dir是一个 list ，内容是该文件夹（例如“25”）中所有的目录的名字
            # files是 list , 内容是该文件夹（例如“25”）中所有的文件(不包括子目录)
    ...
    def __iter__(self):
        ...
        ctr = 0
        for root, file in tuple_files:  # 遍历每一个file
            try:
                it = 0
                # TarFile类对于就是tar压缩包实例
                # 是TarInfo对象的序列
                tar = tarfile.open(os.path.join(root, file), 'r:gz') # 打开一个压缩包
                dir_info = tar.next()
                # next() 就是for方法迭代器调用的, 返回下一个TarInfo对象
                results_file = tar.extractfile(dir_info.name+"/simulationResults.txt")
                # 提取相应对象为一个只读文件对象. member这里可以是文件名或者TarInfo.
                ...
                while(True):
                    ...
                    # Filter traffic intensity 过滤流量强度
                    ...
                    # 在input_file里面提取第一行，找对应目录的graph文件
                    # 连接多个路径名组件，root/graphs/"used_files[1]"
                    s._routing_file = os.path.join(root,"routings",used_files[2])
                    # 在input_file里面提取第一行，找对应目录的routing文件
                    if (s._graph_file in self._graphs_dic):
                        g = self._graphs_dic[s._graph_file]
                    else:
                        g = networkx.read_gml(s._graph_file, destringizer=int)
                        #现在网络文件基本用GML格式。python的networkx包可以读取并解析
                        # 从s._graph_file路径下读取gml文件，把节点等变量都转换成int格式
    ...
    def _process_flow_results(self, s):
        ...
        # 就是simulationResults文件的第一行，用逗号隔开的分别是
        ...
        for i in range(0,len(r), int(math.sqrt(len(r)))):
            ...
            for j in range(i, i+int(math.sqrt(len(r)))):
                ...
                # 具有特定[src, dst]对之间所有聚合流的性能度量的字典

                # 节点 i 和节点 j 之间每个流的性能测量的字典列表
                lst_result_flows = []
                aux_result_flows = f[j].split(':')
                # 针对流的信息：还是simulationResults文件
                ...
                dict_traffic_srcdst = {}
                # From kbps to bps
                # simulationResults文件|后面的以；分割的11个元素，前两个是聚合的流量信息，后面9个元素是聚合流量的测试性能信息
                dict_traffic_agg = {'AvgBw':aux_agg[0]*1000,  # 平均带宽（比特/时间单位）
                                    'PktsGen':aux_agg[1],    # 生成的数据包（数据包/时间单位）
                                    'TotalPktsGen':aux_agg[1]*sim_time}  # 总的数据包数量
                lst_traffic_flows = []
