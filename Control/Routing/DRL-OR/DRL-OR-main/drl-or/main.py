...
def main():
    ...
    log_dir = os.path.expanduser(args.log_dir)  # ~或~user 替换为当前用户的home目录并返回
    eval_log_dir = log_dir + "/eval"
    utils.cleanup_log_dir(log_dir)  # 创建./log/test目录
    utils.cleanup_log_dir(eval_log_dir)   # 创建./log/test/eval目录
    
    model_save_path = args.model_save_path   # 保存模型参数路径：./model/test
    model_load_path = args.model_load_path  # 模型参数加载路径：默认是none，run.sh没有指定
    ckpt_step = args.ckpt_steps  # model save every ckpt_step  # 训练时每个检查点的迭代步骤数10000
    ...
    for i in range(args.num_pretrain_epochs):
        for j in range(args.num_pretrain_steps):
            ...
            with torch.no_grad():
                ...
                # 这里面每次都是act生成一个action，然后envs.next_agent根据action生成当前节点邻居的第一个
                # 每次进入下一轮循环，action和path都会改变
                while curr_agent != None and agents_flag[curr_agent] != 1:
                    ...
                    #start = time.time()
                    # 第一个参数就是当前节点的rollout的obs观察矩阵的当前步数的观察状态（每行代表一步）
                ...
                #上面的while在到达目的点or遇到循环时退出，然后下面这个for将剩下还没来得及学习的节点在当前step的环境下学下动作
            ...
            # 每个节点在当前的step下都学了动作，src的path上的点是按照path环境学的，剩下的是按照step环境学的
        ...
        for k in range(num_agent):
            ...
            if args.use_linear_lr_decay:
                ...
                if args.algo == "acktr":
                    ...
                    # 每训练128个step，也就是1个epoch缩减模型的lr
            if args.algo == 'ppo' and args.use_linear_clip_decay:
                #  args.use_linear_clip_decay初始=False
    ...
    for i in range(num_agent):
        rollout = RolloutStorage(args.num_steps,
                        observation_spaces[i].shape, action_spaces[i],
                        actor_critic.recurrent_hidden_state_size, num_node)
        # 初始化rollout，train的step是512，所以状态矩阵的行数也是512
        rollouts.append(rollout)
        rollouts[i].obs[0].copy_(obses[i])
        rollouts[i].to(device)
    ...
    for j in range(args.num_env_steps):
        ...
        with torch.no_grad(): 
            ...
            while curr_agent != None and agents_flag[curr_agent] != 1:
                ...
                condition_states[curr_agent] = condition_state
                curr_agent, path = envs.next_agent(curr_agent, action)
                # curr_agent已经是下一个节点了，path只存储[下一个节点]
                #  condition_states就是11个condition_state，每个condition_state中，
                #  不在path上的点是none，在path上的点是从src到这个点走过的路，condition_state
            ...
            for k in range(num_agent):
                if agents_flag[k] != 1:
                    ...
                    recurrent_hidden_states[k] = recurrent_hidden_state
                    condition_states[k] = condition_state
                    # 不在path上的点， condition_states里面存的都是[0,0,0...0]
        ...
        # 这里delay就是mininet返回的delay的原始值
        # thoughput是标准化后的值（原值/请求需求）
        # loss也是原值
        print(delta_dist, file=log_dist_files[rtype])
        print(delta_demand, file=log_demand_files[rtype])
        ...
        for k in range(num_agent):
            ...
            if agents_flag[k] == 1:
                masks = torch.tensor([1.])  # 标志k这个节点是否在生成的path上，在masks就=1，不在=0
            else:
