...
class Controller(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 定义配置文件的信息，由run.sh传入toponame = "Abi"
        ...
        print("loading topoinfo finished")  # 加载topo信息完成
        ...
        for i in range(self.node_num):
            self.link_port[i + 1] = {}  # [u][v]里是u交换机到v的port
        switch_port_counter = [1] * self.node_num   # 初始化num长度里面为1的数组，存放每个交换机的port计数
        ...
        self.switch_host_port = {}   # 存放交换机到host的port
        for i in range(self.node_num):
    ...
    def install_rules(self):
        ...
        ''' 接受来自simenv的连接，addr是其IP地址 '''
        print("Connection address:", addr)
        # addr是simenv的端口

        while len(self.datapaths) < self.node_num:  # datapath数组还没加载满？
            hub.sleep(5)
        ...
        while True:
            ...
            path = data_js['path']  # 这个是直接一个中心agent？？
            ...
            temp = {}  # 相当于条件状态数组吧，只不过一次加载完
            for i in range(len(path)):
                temp[path[i]] = 1    # path上的交换机都在temp里置1
    ...
    def _packet_in_handler(self, ev):
        ...
        # miss_send_length：指示交换机遇到无法处理的数据包时，向控制器发送消息的最大字节数
